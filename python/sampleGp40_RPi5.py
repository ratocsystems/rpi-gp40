...
from gpiozero import LED # 絶縁電源/DIO制御用
from gpiozero import Button # アラーム検出用
from gpiozero import DigitalOutputDevice # CS制御用
...
def xfer_spiadc( wd ):
    pin_CS = DigitalOutputDevice(pin=ADCS)
    pin_CS.off()			     # SPI CS0='L' GPIO端子として制御する
    rd = spi.xfer(wd)
    pin_CS.on()			         # SPI CS0='H'
    return rd
...
def callback_adalarm(din):
    ...
    if( rdat[2]==0 ):                         # アラームなし＝DIN入力(H→L変化)あり
        ...
        print("！デジタル入力DIN(H→L変化)検知！ ch0[%03X] Enterを押してください" % adadt )    # 検知時のch0 ADCデータ表示
    else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        d_out = LED(DOUT,active_high=True,initial_value=True) # 絶縁電源/DIO制御用
        button = Button(DIN)        # アラーム検出用
                    
        # RPi-GP40初期設定
        pwr = LED(27)               # RPi-GP40の絶縁電源制御
        pwr.on()
    ...
    # 例外処理
         ...
         print( "中断しました" )      # 中断
    except Exception as e:          # その他の例外発生時
         print( f"エラー:{e}" )      # エラー
